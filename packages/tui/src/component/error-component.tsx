      ...
      <box width={contentWidth()} flexGrow={1} flexDirection="column" paddingTop={1} paddingBottom={1} gap={1}>
        ...
        <box flexDirection="column" alignItems="center" flexShrink={0}>
          <text attributes={TextAttributes.BOLD} fg={colors.text}>
            advcode crashed — spectacularly
          </text>
          <Show when={showSubtext()}>
            <text fg={colors.muted}>Something went sideways. Here's the damage report.</text>
          </Show>
        </box>
        ...
        <Show when={showFooter()}>
          <box flexDirection="column" alignItems="center" flexShrink={0}>
            <text fg={colors.muted}>
              {copied()
                ? "Report copied — go open a GitHub issue and paste this in."
                : "Copy the report and open a GitHub issue so we can fix this."}
            </text>
            <text fg={colors.muted}>advcode {InstallationVersion}</text>
          </box>
        </Show>
      </box>
    </box>
...
function buildIssueURL(message: string, stack: string) {
  ...
  url.searchParams.set(
    "reproduce",
    "Reported automatically from the advcode crash screen. If you can, describe what you were doing when it crashed.",
  )
  ...
  const marker = "\n... (truncated)"
  const head = `ADVCode crashed. Here's what happened:\n\n**Error:** ${message}\n\n**Stack trace:**\n`
